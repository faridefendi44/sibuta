...
<div id="sidebar" class="relative    bg-[#BF3131] text-white   left-0 top-0  transition-all duration-300 ease-in-out">
    <div class="p-10   bg-[#BF3131]   flex items-center h-16 p-4">
        <div class="w-2/3 text-md  w-full flex my-auto space-x-3">
            ...
            @if(Auth::check())
            <h1 id="greetingText" class="">Selamat Datang {{auth()->user()->name}}</h1>
            @else
    ...
    <div class="p-4">
        ...
        <ul class="mt-4 space-y-5 text-white   " id="sidebarMenu">
            ...
            @if (Auth::check())
                <li>
                    <a href="{{ url('pegawai/') }}"
                        class="parent  {{ Request::path() == 'pegawai' ? 'bg-[#7D0A0A]' : '' }}  hover:bg-[#7D0A0A]  -ml-2 p-4 w-auto h-12 mt-2 flex items-center rounded-md duration-300 cursor-pointer   font-semibold shadow relative">
                        <h1
                            class="{{ Request::path() == 'pegawai' ? 'active' : 'opacity-0' }}  bg-red-400

                    bg-[#345C6D]    w-1 h-full  absolute left-0 child">
                        </h1>
                        <svg class="w-6 h-6" viewBox="0 0 1024 1024" fill="#fff" class="icon" version="1.1"
                        ...
                        <span id="show" class="text-[12px] md:text-[18px] ml-7 ">Kelola Pegawai</span>
                    </a>
                </li>
            @endif
            <li class="relative">
